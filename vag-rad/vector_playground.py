...
def plot_shifted_graph(graph: nx.MultiDiGraph, debug_marker = False, plot_arrow_heads = False, plot_original_graph=False) -> leafmap.Map:
    ...
    graph = graph.copy()
    map = leafmap.Map(location=[49.454446, 11.112065], zoom_start=15, crs='EPSG3857')
    ...
    for edge in graph.edges(data=True):
        ...
        # edges at intersections have no reversed attribute because they are only traversed in one direction
        if reversed is None:
            color = 'green'
            try:
                start = [graph.nodes[s]['y_reversed'], graph.nodes[s]['x_reversed']]
        ...
        folium.PolyLine([start, dest], color=color).add_to(map)
        ...
        if plot_arrow_heads:
            ...
            folium.RegularPolygonMarker(location=arrow_pos, color=color, fill=True, fill_color=color, fill_opacity=1, number_of_sides=3, rotation=rot, radius=5).add_to(map)

        if plot_original_graph:
            start = [graph.nodes[s]['y'], graph.nodes[s]['x']]
            dest = [graph.nodes[d]['y'], graph.nodes[d]['x']]
            folium.PolyLine([start, dest], color='black').add_to(map)
    return map

plot_shifted_graph(graph, plot_original_graph=True, plot_arrow_heads=True, debug_marker=True)
# %%
